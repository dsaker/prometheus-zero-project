...
float humidity, celsius, c_heat_index, fahrenheit, f_heat_index;
...
void handle_http_metrics() {
    ...
    static char const *response_template =
        ...
        "# HELP " PROM_NAMESPACE "_air_temperature_fahrenheit Air temperature.\n"
        "# TYPE " PROM_NAMESPACE "_air_temperature_fahrenheit gauge\n"
        "# UNIT " PROM_NAMESPACE "_air_temperature_fahrenheit \u00B0C\n"
        PROM_NAMESPACE "_air_temperature_fahrenheit %f\n"
        ...
        PROM_NAMESPACE "_air_heat_index_fahrenheit %f\n";

    read_sensors();
    if (isnan(humidity) || isnan(celsius) || isnan(c_heat_index) || isnan(fahrenheit) || isnan(f_heat_index)) {
...
void read_sensors(boolean force) {
    ...
    f_heat_index = (c_heat_index*1.8) + 32;
}
...
void read_temperature_sensor() {
    ...
    if (result) {
        celsius += TEMPERATURE_CORRECTION_OFFSET;
    } else {
...
void read_heat_index() {
    if (!isnan(humidity) && !isnan(celsius)) {
        c_heat_index = dht_sensor.computeHeatIndex(celsius, humidity, false);
    } else {
        c_heat_index = NAN;
    }
}
